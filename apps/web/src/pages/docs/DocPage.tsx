      ...
      <header className="sticky top-16 z-40 border-b border-[var(--color-border)] bg-[color-mix(in_srgb,var(--color-bg-primary)_94%,transparent)] backdrop-blur-xl">
        <div className="flex h-16 items-center gap-3 px-4 lg:px-6">
          <button
            ...
            className="nowen-focus flex h-10 w-10 items-center justify-center rounded-lg border border-[var(--color-border)] lg:hidden"
            aria-label="打开目录"
          >
            <Menu className="h-5 w-5" />
          </button>
          <Link to={`/docs/${helpCenter.slug}`} className="min-w-0 font-semibold text-[var(--color-text-primary)]">
            <span className="truncate">{helpCenter.name} 帮助中心</span>
          </Link>
          <div className="relative ml-auto w-full max-w-xs">
            ...
            <input
              ...
              placeholder="搜索本项目"
              className="nowen-focus h-10 w-full rounded-full border border-[var(--color-border)] bg-[var(--color-bg-secondary)] pl-9 pr-4 text-sm"
            />
            {keyword.trim().length >= 2 && (
              <div className="absolute right-0 top-12 z-50 w-[min(26rem,calc(100vw-2rem))] overflow-hidden rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-primary)] shadow-xl">
                {search.isPending ? (
                  <p className="p-4 text-sm text-[var(--color-text-muted)]">正在搜索…</p>
                ) : searchItems.length ? (
                  searchItems.slice(0, 8).map((item) => (
                    <Link
                      key={item.id}
                      to={`/docs/${item.spaceSlug}/${item.path}`}
                      className="nowen-focus block border-b border-[var(--color-border)] px-4 py-3 last:border-b-0 hover:bg-[var(--color-glass-hover)]"
                    >
        ...
        <main className="min-w-0 px-5 py-10 sm:px-8 lg:px-12 lg:py-12">
          <article className="mx-auto max-w-[820px]">
            <nav className="mb-6 flex flex-wrap items-center gap-2 text-xs text-[var(--color-text-muted)]" aria-label="面包屑">
              <Link to="/docs" className="hover:text-[var(--color-primary)]">帮助中心</Link>
              <span>/</span>
            ...
            <header className="border-b border-[var(--color-border)] pb-7">
              <h1 className="text-3xl font-semibold tracking-[-0.035em] text-[var(--color-text-primary)] sm:text-4xl">
                {activePage.title}
              </h1>
              ...
              <div className="mt-4 flex flex-wrap items-center gap-3 text-xs text-[var(--color-text-muted)]">
                <span>最后更新：{activePage.updatedAt.slice(0, 10)}</span>
                {activePage.editUrl && (
                  <a href={activePage.editUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 hover:text-[var(--color-primary)]">
                    在 GitHub 编辑 <ExternalLink className="h-3 w-3" />
                  </a>
                )}
              </div>
            </header>
            ...
            <div className="py-8">
              <Markdown content={normalizeDocsMarkdown(activePage.contentMd)} />
            </div>

            <section className="mt-8 rounded-2xl border border-[var(--color-border)] bg-[var(--color-bg-secondary)] p-5">
              {feedbackSent ? (
                <p className="text-center text-sm text-emerald-600">感谢反馈，我们会继续完善帮助文档。</p>
              ) : (
        ...
        <div className="fixed inset-0 z-[70] lg:hidden">
          ...
          <aside className="absolute inset-y-0 left-0 w-[min(86vw,320px)] overflow-y-auto bg-[var(--color-bg-primary)] shadow-2xl">
            <div className="flex h-16 items-center justify-between border-b border-[var(--color-border)] px-4">
              <p className="font-semibold text-[var(--color-text-primary)]">{helpCenter.name} 帮助中心</p>
